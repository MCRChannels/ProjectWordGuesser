...
public class WordleGUI extends JFrame implements KeyListener{
    ...
    private void loadCorrectWordCount() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("guessed_words.log"))) {
            reader.readLine(); // skip first line
            String line;
            while ((line = reader.readLine()) != null) {
                if (!line.trim().isEmpty()) {  // Check if line is not empty or just only spaces
                    correctWordCount++; // Count word 
                }
            }
        } catch (IOException e) {
    ...
    private void saveCorrectWord(String word) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("guessed_words.log", true))) {
            writer.write(word);
