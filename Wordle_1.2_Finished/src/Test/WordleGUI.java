...
public class WordleGUI extends JFrame implements KeyListener{
    ...
    private void createModeSelector() {
        ...
        wordCountLabel = new JLabel("Correctly Guessed word : " + correctWordCount); // word count
        ...
        modePanel.add(wordCountLabel);
        modePanel.add(Box.createRigidArea(new Dimension(0, 25)));

        loadCorrectWordCount();
    ...
    private void loadCorrectWordCount() {
        correctWordCount = 0;  // Reset count
        try (BufferedReader reader = new BufferedReader(new FileReader("Wordle\\src\\Test\\guessed_words.log"))) {
            reader.readLine(); // skip first line
            while (reader.readLine() != null) {
                correctWordCount++; // count word in guessed_word.log
                if (reader.readLine() == " ") { // if in log flie have space 
                    reader.readLine();
                }
        ...
        wordCountLabel.setText("Correctly Guessed word : " + correctWordCount);  // Update label
    }

    private void saveCorrectWord(String word) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("Wordle\\src\\Test\\guessed_words.log", true))) {
            writer.write(word);
    ...
    private void returnToMenu() {
        isTimerPaused = true; // pause Timer
    ...
    private void createKeyboard() {
        ...
        for (int i = 0; i < rows.length; i++) {
            ...
            if (i == 2) {
                ...
                addFocusListener(backspaceButton);
    ...
    private void addFocusListener(JButton button) {
        button.addFocusListener(new FocusListener() {
            ...
            public void focusGained(FocusEvent e) {
                WordleGUI.this.requestFocusInWindow();
            }
    ...
    private void endGame(Map<String, Object> gameState) {
        ...
        if (gameState.get("won").equals(true)) {
            ...
            correctWordCount++;
            wordCountLabel.setText("Correctly Guessed word : " + correctWordCount);  // Update label
            saveCorrectWord(((String) gameState.get("targetWord")).toLowerCase());  // Save correct word
